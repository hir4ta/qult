...
flowchart TB
    ...
    Adversarial -- "FAIL / aggregate < 34 / dimension < 4" --> Claude
    Adversarial -- "PASS + aggregate ≥ 34/40 + 全次元 ≥ 4" --> Done["Commit"]
...
| **サブエージェント** (enforcement) | SubagentStop | レビュー出力検証、次元フロア + 合計閾値強制 (34/40)、スコアバイアス検出、read-only 強制、Agentic Flywheel |
...
| 4 | **Adversarial Reviewer** | Haiku | EdgeCases + LogicCorrectness | サイレント障害、off-by-one、型変換、競合状態はないか |
...
| キー | 型 | デフォルト | 説明 |
| --- | --- | --- | --- |
| `review.score_threshold` | number | 34 | 4段階レビュー合格に必要な合計スコア（最大40） |
...
環境変数でのオーバーライド: `QULT_REVIEW_SCORE_THRESHOLD`, `QULT_REVIEW_MAX_ITERATIONS`, `QULT_REVIEW_REQUIRED_FILES`, `QULT_REVIEW_DIMENSION_FLOOR`, `QULT_REQUIRE_HUMAN_APPROVAL`, `QULT_GATE_OUTPUT_MAX`, `QULT_GATE_DEFAULT_TIMEOUT`
...
| スコア例 | 合計 | 結果 |
| --- | --- | --- |
| 5+4+5+4+4+4+4+4 | 34 | 合格。堅実なコードはこのラインで通過する |
| 4+4+4+4+4+4+4+4 | 32 | 不合格。一貫した「良好」でも足りない。AI レビュアーは甘くなりがちなため |
...
| 4+4+4+4+5+5+4+4 | 34 | 合格。全次元がフロア以上 |
...
| **adversarial-reviewer** | Haiku | Devil's advocate: エッジケース、ロジックエラー、サイレント障害 (EdgeCases, LogicCorrectness) |
