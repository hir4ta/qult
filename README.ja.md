...
![Version](https://img.shields.io/badge/version-0.21.0-7fbbb3?style=flat-square)
...
> プロンプトは提案。hooks は強制。
> qult は 7 hooks + MCP server + 3段階独立レビューで、品質低下を **お願い (advisory) ではなく exit 2 (DENY) で止める**。
...
**harness engineering** — [OpenAI](https://openai.com/index/harness-engineering/) が命名し、[Martin Fowler](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html) が体系化した新興分野 — はこの問題に対処する:

> **Agent = Model + Harness.** 不変条件を強制し、実装を細かく管理しない。

研究がこのアプローチを裏付ける:

- **TDAD** ([arXiv:2603.17973](https://arxiv.org/abs/2603.17973)): プロンプトのみで TDD 指示を追加すると、リグレッションが 6% → 10% に**悪化**。構造的な強制で 1.8% に低減。
- **Specification as Quality Gate** ([arXiv:2603.25773](https://arxiv.org/abs/2603.25773)): AI が AI をレビューするのは構造的に循環的 — 相関したエラーが増幅する。決定論的ゲートを先に、AI レビューは残余のみ。

qult はこのアーキテクチャを実装: **決定論的ゲート (lint, typecheck) → 実行可能な仕様 (test) → AI レビュー (残余のみ)**。
...
| **構造ベース** (qult) | Hook exit 2 (DENY) | 構造的 — モデルが回避不可能 |

Fowler の分類で: qult の hooks は **センサー** (観察して修正するフィードバック制御)、skills と CLAUDE.md は **ガイド** (行動前に方向づけるフィードフォワード制御)。両方が必要であり、qult は両方を提供する。
...
flowchart TB
    ...
    Spec --> Quality
    Quality --> Security
    Security -- "FAIL / aggregate < 24 / dimension < 3" --> Claude
    Security -- "PASS + aggregate ≥ 24/30 + 全次元 ≥ 3" --> Done["Commit"]
...
| レビュー PASS だが合計スコア低い | **block** — 傾向分析付きで再レビュー (最大3回) |
| レビュー PASS だが個別次元がフロア未満 | **block** — 例: Security=2 は合計が高くてもブロック |
...
| **サブエージェント** (enforcement) | SubagentStop | レビュー出力検証、次元フロア + 合計閾値強制 (24/30) |
...
1. **次元フロア** (デフォルト: 3/5) — 個別の次元がフロア未満なら、合計スコアに関係なく即座にブロック。「優秀なコードだがセキュリティが壊滅的」な状態を防ぐ。
2. **合計閾値** (デフォルト: 24/30) — 全3ステージ完了後、6次元の合計スコアが閾値を満たす必要がある。最大3イテレーション。
...
| `review.dimension_floor` | number | 3 | 次元ごとの最低スコア (1-5)。フロア未満の次元があると合計に関係なくブロック |
| `gates.output_max_chars` | number | 2000 | ゲート出力の最大文字数 (超過分は truncate) |
| `gates.default_timeout` | number | 10000 | ゲートコマンドのタイムアウト (ms) |

環境変数: `QULT_REVIEW_SCORE_THRESHOLD`, `QULT_REVIEW_MAX_ITERATIONS`, `QULT_REVIEW_REQUIRED_FILES`, `QULT_REVIEW_DIMENSION_FLOOR`, `QULT_GATE_OUTPUT_MAX`, `QULT_GATE_DEFAULT_TIMEOUT`
...
**次元フロア** (デフォルト 3/5):
- 5+5+5+5+2+2 = 24: 合計は通過するが **ブロック** — Vulnerability=2 または Hardening=2 がフロア未満
- 3+3+4+4+5+5 = 24: 通過 — フロア未満の次元なし

次元フロアは、一部の観点で高スコアを取ることで他の観点（特にセキュリティ）の致命的な弱点を隠す危険なパターンを防ぐ。設定変更可能: プロトタイプなら下げる (`"dimension_floor": 2`)、本番APIなら上げる (`"dimension_floor": 4`)。
...
|-------------|-------------|------|
| **hooks** | センサー (フィードバック) | 強制 — 品質違反を DENY (exit 2) でブロック |
