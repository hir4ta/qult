...
> プロンプトは提案。hooks は強制。
> qult は 7 hooks + MCP server + 4段階独立レビューで、品質低下を **exit 2 (DENY) で止める**。
...
Anthropic の [Harness Design](https://www.anthropic.com/engineering/harness-design-long-running-apps) 記事は2つの根本原因を特定している: **コンテキスト一貫性の喪失**（コンテキストが埋まるにつれモデルが劣化する）と**自己評価バイアス**（エージェントが凡庸な自分の成果物を自画自賛する）。解決策は、生成と評価を分離し、品質を構造的に強制すること。

[Martin Fowler](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html) が体系化した **harness engineering** はこの問題をより広くフレーム化する:
...
flowchart TB
    ...
    subgraph Evaluator["4段階 Evaluator"]
    ...
    Adversarial -- "FAIL / aggregate < 30 / dimension < 4" --> Claude
    Adversarial -- "PASS + aggregate ≥ 30/40 + 全次元 ≥ 4" --> Done["Commit"]
...
| ファイル内/ファイル間のコード重複 | **warn/block**: 重複検出（intra-file はブロック、cross-file はアドバイザリー、8回でエスカレーション） |
| レビュアーが「問題なし」と言うがdetectorが問題検出 | **block**: cross-validation がレビュアーの主張と ground truth の矛盾を検出 |
| レビュアーが無断でコミット | **block**: read-only 強制が不正なコミットを検出 |
| 人間の承認が未記録（設定時） | **block**: opt-in human review gate が architect の承認を要求 |

## 完全なワークフロー

qult は 12 スキルと 7 エージェントで完全な開発ワークフローを提供する。

```
/qult:explore        → architect にインタビュー、設計探索
/qult:plan-generator → 構造化実装計画の生成
    [Plan mode]      → architect がレビュー・承認
/qult:review         → 4段階独立レビュー (Spec → Quality → Security → Adversarial)
...
| **サブエージェント** (enforcement) | SubagentStop | レビュー出力検証、次元フロア + 合計閾値強制 (30/40)、スコアバイアス検出、read-only 強制、Agentic Flywheel |
...
| get_detector_summary | 計算検出器の統合結果を返す（レビューの ground truth） |
...
| record_review | レビュー完了を aggregate score とともに記録 |
| record_test_pass | テスト pass を記録 |
| record_stage_scores | ステージごとのレビュー次元スコアを記録 |
| record_human_approval | architect の人間レビュー承認を記録（opt-in） |
| get_harness_report | ハーネス効果分析: ゲート検出率、レビュー傾向、未使用ゲート検出 |
| get_handoff_document | セッション引き継ぎドキュメント（プラン進捗、pending fixes、変更ファイル） |
| get_metrics_dashboard | 履歴メトリクス可視化: ゲート失敗、セキュリティ警告、レビュースコア |
...
| `/qult:review` | 4段階独立コードレビュー (Spec + Quality + Security + Adversarial) |
...
qult のレビュー (`/qult:review`) は4つの専門レビュアーを順番にスポーンし、計算検出器の結果を ground truth として注入する。

| ステージ | エージェント | モデル | 評価次元 | 焦点 |
| --- | --- | --- | --- | --- |
| 1 | **Spec Reviewer** | Sonnet | Completeness + Accuracy | 実装が Plan に合致しているか。コンシューマは更新されているか |
| 2 | **Quality Reviewer** | Opus | Design + Maintainability | コード設計は適切か。エッジケースは処理されているか |
| 3 | **Security Reviewer** | Opus | Vulnerability + Hardening | インジェクションリスクはないか。多層防御は適用されているか |
| 4 | **Adversarial Reviewer** | Sonnet | EdgeCases + LogicCorrectness | サイレント障害、off-by-one、型変換、競合状態はないか |

各エージェントが2次元を採点（各1-5）。合計 **8次元 / 40点満点**。

レビュアー起動前に `get_detector_summary` で決定論的な検出器結果（セキュリティパターン、デッドインポート、重複、テスト品質）を収集し ground truth として各ステージに注入。cross-validation がレビュアーの主張と矛盾を検出する。
...
2. **合計閾値**（デフォルト: 30/40）
   - 全4ステージ完了後、8次元の合計スコアが閾値を満たす必要がある
   - 最大3イテレーション
...
| キー | 型 | デフォルト | 説明 |
| --- | --- | --- | --- |
| `review.score_threshold` | number | 30 | 4段階レビュー合格に必要な合計スコア（最大40） |
...
| `review.require_human_approval` | boolean | false | architect の `record_human_approval` 呼び出しをコミット前に要求（opt-in） |
| `plan_eval.score_threshold` | number | 12 | 計画評価の合格スコア（最大15） |
| `gates.output_max_chars` | number | 3500 | ゲート出力の最大文字数（超過分は truncate） |
| `gates.default_timeout` | number | 10000 | ゲートコマンドのタイムアウト（ms） |
| `escalation.security_threshold` | number | 10 | セキュリティ警告がこの数に達するとセッション完了をブロック |
| `escalation.drift_threshold` | number | 8 | 規約ドリフト警告の閾値 |
| `escalation.test_quality_threshold` | number | 8 | テスト品質警告の閾値 |
| `escalation.duplication_threshold` | number | 8 | 重複検出警告の閾値 |

環境変数でのオーバーライド: `QULT_REVIEW_SCORE_THRESHOLD`, `QULT_REVIEW_MAX_ITERATIONS`, `QULT_REVIEW_REQUIRED_FILES`, `QULT_REVIEW_DIMENSION_FLOOR`, `QULT_REQUIRE_HUMAN_APPROVAL`, `QULT_PLAN_EVAL_SCORE_THRESHOLD`, `QULT_GATE_OUTPUT_MAX`, `QULT_GATE_DEFAULT_TIMEOUT`, `QULT_ESCALATION_SECURITY`, `QULT_ESCALATION_DRIFT`, `QULT_ESCALATION_TEST_QUALITY`, `QULT_ESCALATION_DUPLICATION`

<details>
<summary><strong>レビュースコア閾値の根拠</strong></summary>

4段階レビューは8つの観点 (Completeness, Accuracy, Design, Maintainability, Vulnerability, Hardening, EdgeCases, LogicCorrectness) を各1-5で採点する。

**合計閾値**（デフォルト 30/40）:

| スコア例 | 合計 | 結果 |
| --- | --- | --- |
| 4+4+4+4+4+4+4+4 | 32 | 合格。一貫した「良好」は余裕で通過する |
| 4+4+3+4+4+4+4+4 | 31 | 合格。1つの弱い次元は許容される |
| 4+3+4+3+4+4+3+4 | 29 | 不合格。複数の弱い次元がある |
| 3+3+3+3+3+3+3+3 | 24 | 大幅に不合格。全体的な品質不足は確実に検出される |
| 5+5+4+4+5+5+4+5 | 37 | 余裕で合格 |

**次元フロア**（デフォルト 4/5）:

| スコア例 | 合計 | 結果 |
| --- | --- | --- |
| 5+5+5+5+3+3+5+5 | 36 | 合計は通過するが **ブロック**。Vulnerability=3 がフロア未満 |
| 4+4+4+4+4+4+4+4 | 32 | 合格。全次元がフロア以上 |
...
**スコアバイアス検出**: 8次元すべてが同一スコア、またはスコアの分散が低い（レンジ < 2）場合に警告する。テンプレート的な AI レビュー応答を検出するための仕組み。
...
| **コード重複検出** | AI が8倍のコード重複を生成する (GitClear 2025) | PostToolUse がファイル内重複ブロック（4行以上、ブロッキング）とファイル間重複（アドバイザリー）を検出。8回でエスカレーション |
| **Ground truth 注入** | LLM が LLM をレビューするのは循環的 — 相関エラーが増幅 | 計算検出器の結果をレビュアーコンテキストに ground truth として注入。cross-validation が矛盾をブロック |
| **Read-only レビュアー強制** | レビュアーが read-only 制約にもかかわらずファイル変更やコミットを実行 | SubagentStop が HEAD をセッション状態と比較して不正なコミットを検出 |
| **Human review ゲート** | AI レビューだけでは不十分（研究のコンセンサス） | opt-in `require_human_approval` 設定。architect が `record_human_approval` を呼ぶまでコミットをブロック |
| **コードベース連動 explore** | AI がプロジェクト固有でなく一般的な質問をする | Phase 0 でコードベースを走査し、関連する型やパターンに基づいた質問を生成 |
| **Impact Analysis** | AI が直接テストファイルのみ実行し、消費者のリグレッションを見逃す | import グラフで変更ファイル → importer → テストファイルを追跡。下流の影響を検出 |
| **Review tier** | 1ファイル修正にも20ファイル機能にも同じ4-stage レビュー | 段階的レビュー: skip (1-2), light (3-4), standard (5-7 or plan), deep (8+ with e2e) |
| **Harness effectiveness report** | ゲートが実際に問題を検出しているか不明 | メトリクス履歴を分析: ゲート検出率、レビュー傾向、未使用ゲート推奨 |
| **Context handoff** | コンテキストリセットでセッション状態が失われる | プラン進捗、pending fixes、変更ファイルを含む構造化引き継ぎドキュメント |
...
| **spec-reviewer** | Sonnet | 実装が計画に合致しているか検証 (Completeness, Accuracy) |
...
| **adversarial-reviewer** | Sonnet | Devil's advocate: エッジケース、ロジックエラー、サイレント障害 (EdgeCases, LogicCorrectness) |
...
- [Anthropic: Harness Design](https://www.anthropic.com/engineering/harness-design-long-running-apps) — Generator-Evaluator パターン、自己評価バイアス、コンテキスト一貫性喪失
- [Anthropic: Effective Harnesses](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) — 長時間エージェントセッション設計、構造化ハンドオフ
