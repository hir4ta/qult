...
Used by Phase 1 of `/alfred:develop`. Spawn all 3 in a single message.

Note: develop's spec phase uses knowledge-base + codebase context only (no web search).
For web-research-enhanced planning, use `/alfred:plan` directly.
...
using `spec` action=update. The Mediator does NOT call the spec tool directly.
