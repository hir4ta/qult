...
Used by Phases 2, 4, and 5 of `/alfred:develop`. All review agents output
...
- Verdict is always `PASS` or `NEEDS_FIXES` (no other values)
...
```
You are a spec reviewer. Perspectives: Correctness/Completeness and Testability.

Read the spec independently using the spec tool (action=status).
...
```
You are a spec reviewer. Perspectives: Security/Privacy and Performance.
...
Read the spec independently via spec tool (action=status) for context.
...
### Agent C: Architecture + Operability

```
You are a code reviewer. Perspectives: Architecture Fit and Operability.
...
Read the spec independently via spec tool (action=status).
...
{"verdict": "PASS" or "NEEDS_FIXES", "findings": [...]}
