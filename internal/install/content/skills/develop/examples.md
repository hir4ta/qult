...
```
/alfred:develop add-config-option Add a per-project configuration option for custom knowledge sources
```
...
Phase 1: Spec Creation
  ...
  → Parent writes spec via: spec action=update (requirements.md, design.md, decisions.md)
...
Output:
  ...
  2. After resolving: run `/alfred:develop add-config-option` to resume
  ### Unresolved findings
...
```
/alfred:develop add-config-option
