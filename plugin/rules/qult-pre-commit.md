...
2. **Check session status** — call `mcp__plugin_qult_qult__get_project_status`. Verify:
   - `pending_fixes` is empty (or addressed)
   - `test_passed_at` is recent
   - `review_completed_at` is set if the change is non-trivial
3. **Review** — if 5 or more source files changed, OR a plan is active, run `/qult:review` first. After review passes, the MCP server records the score automatically; verify via `get_project_status`.
...
Only the steps above apply when source code changed (e.g. `.ts`, `.py`, `.go`, `.rs`). For non-source commits (version bump, README, lockfile-only), skip the test/review steps — but still confirm `get_project_status` shows no blocking `pending_fixes`.
